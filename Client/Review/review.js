document.addEventListener("DOMContentLoaded", () => {
  ...
  function fetchBlogs() {
    fetch(
      `../../Server/api/getallblogsUnderReview.php?limit=${limit}&offset=${offset}`
    )
            ...
            postElement.innerHTML = `
              ...
              post.title
                                ...
                                <div class="blog-actions">
                                    <button class="accept-btn" onclick="updateStatus(${
                                      post.id
                                    }, 'accepted')">Accept</button>
                                    <button class="decline-btn" onclick="updateStatus(${
                                      post.id
      ...
      .catch((error) => {
        ...
        document.querySelector(
          ".blog-wrapper"
        ).innerHTML = `<p>Error loading blogs: ${error.message}</p>`;
      });
  }
...
function updateStatus(postId, newStatus) {
  fetch("../../Server/api/updateblogStatus.php", {
    ...
    .then((data) => {
      ...
      alert(data.message || `Post status updated to ${newStatus}`);

      // Remove the post element from the DOM if update was successful
